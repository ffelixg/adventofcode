...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec  2 17:19:32 2023

@author: felix
"""
data = ("""Game 1: 7 red, 8 blue; 6 blue, 6 red, 2 green; 2 red, 6 green, 8 blue; 9 green, 2 red, 4 blue; 6 blue, 4 green
...
import re
res = re.search("Game (?P<nr>[0-9])+: (?P<result>.*)", data)
res.span(2)
res.groupdict()

reg = re.compile("Game ([0-9]+): (.*)")
reg2 = re.compile(r"(\d+) red|(\d+) blue|(\d+) green")
res = re.findall(reg, data)

game = res[-1][-1]
re.findall(r"(\d+) red;?", game)
...
final_sum = 0
for nr, games in re.findall(reg, data):
    winnable = True
    for game in games.split("; "):
        for red, blue, green in re.findall(reg2, game):
            if red and int(red) > 12: winnable = False
            if green and int(green) > 13: winnable = False
            if blue and int(blue) > 14: winnable = False
    if winnable:
        final_sum += int(nr)
...
for nr, games in re.findall(reg, data):
    ...
    for game in games.split("; "):
        for red, blue, green in re.findall(reg2, game):
            min_red = max(min_red, int(red) if red else 0)
            min_blue = max(min_blue, int(blue) if blue else 0)
            min_green = max(min_green, int(green) if green else 0)
    final_sum += min_red * min_blue * min_green
